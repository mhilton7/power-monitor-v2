...
Rate-source revisions and artifacts reject updates and deletes in both ORM and PostgreSQL triggers. Candidate source, normalized values, validation evidence, diff, exact-home manual identity, and canonical digest are immutable. Exact-home review rows permit only `reviewed -> published -> activated` or `reviewed -> rejected`; PostgreSQL and ORM guards reject rollback, linkage/timestamp rewrites, and deletion. Review lifecycle state is stored separately per exact home, so a decision for one home cannot publish, activate, reject, or disclose another home's candidate workflow. Every run records:

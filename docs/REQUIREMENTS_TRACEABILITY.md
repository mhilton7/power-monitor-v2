...
YAML. Candidate rc.5 binds the audited source to migration head
`20260815_0011`, coordinated firmware rc.5, and generated OpenAPI SHA-256
...
|---|---|---|
| 0 | Fixed product/protocol/time defaults in `backend/app/constants.py`; four fixed GHCR image names and generated release-manifest contract | implemented and statically validated; public rc.1/rc.3 GHCR digests and non-released rc.4 image digests are immutable version-specific evidence, while rc.5 is the coordinated audited candidate |
| 1 | `docs/BASELINE_AUDIT.md`, `docs/DEPENDENCY_AUDIT.md`, `docs/MIGRATION.md`; legacy repositories preserved read-only | audited; coordinated rc.1/rc.3 releases are public, rc.2 and rc.4 server publication failed closed at different gates, and candidate rc.5 awaits its own immutable publication evidence |
...
| 5 | Independent native ESP-IDF 6.0.2 firmware repository and exact dependency pins | historical rc.1 snapshot built twice byte-identically at commit `5dea90d`; signed public firmware rc.1, rc.2, rc.3, and rc.4 candidates exist; coordinated rc.5 metadata and physical evidence remain pending |
...
| 12 | `pm-protocol/1.0.0`, HMAC/HKDF/replay/dedupe contracts and generated schemas | rc.3 and failed-publication rc.4 validation is historical; candidate rc.5 keeps the protocol unchanged and requires firmware rc.5 to declare OpenAPI SHA-256 `66b4e1cfb0f5a5797dadd9a8783ff0b192ca416d1f4264c135a4e380b2b94591` |
...
| 14 | OTA metadata/download/manifest/queue/rollback implementation and local release pack | historical rc.1 build/contract evidence and signed public firmware rc.1 through rc.4 releases exist; marked-unit OTA rollback remains pending |
| 15 | FastAPI/PostgreSQL/React/Caddy Compose structure and exact dependency pins | implemented; public rc.1/rc.3 GHCR images and non-released rc.4 images are historical evidence; current rc.5 source retains the exact API-image one-shot initializer and requires its own release build/scan |
| 16 | immutable PZEM ingestion/cost lineage, bill separation, and frozen explicit migrations through current `20260815_0011` | post-rc.3 source added 0008 fail-closed raw/permanent-loss overlap guards and 0010 permanent-loss UPDATE/DELETE immutability without rewriting evidence; the real PostgreSQL 17 clean chain through 0011 and 0011-to-0010-to-head exercise passed locally and rc.4's tagged forward-upgrade gate passed; historical rc.3 remains at `20260813_0007`, while rc.5 publication and rollback remain separate |
| 17 | sessions/MFA/CSRF/throttling/roles/last-owner/home scope/server permissions, including authorized first-sensor home discovery with no enrolled devices and explicit multi-home selection | public rc.3 already exposed authorized scopes with the sensor list before first enrollment; post-rc.3 source adds the permission-independent `/home-scopes` authority and exact-home isolation, covered by backend and frontend zero/one/multiple-scope tests and carried unchanged into rc.5 |
...
| 25 | exact services, internal DB, HTTPS only, UIDs/ACLs, file secrets, hardening, and renderer | published rc.1/rc.3 seven-service smokes are historical; rc.4 exercised the eight-service model but failed the runtime-recovery initializer invariant, and rc.5 repairs that exact check; rc.5 tagged and target-TrueNAS execution remain pending |
| 26 | pinned-action CI/security/release/stable workflows, public-package checks, OCI labels, SBOM/scan/attestations | public rc.1/rc.3 releases and packages exist; rc.2 published none; rc.4 passed the workflow's named `Mandatory release gates` job, scans, four image publications, attestations, and anonymous access but failed deployment smoke before assembly; rc.5 requires its own immutable execution |
| 27 | server/frontend/deployment/firmware gates and evidence schemas | published rc.1/rc.3 gate and smoke evidence is immutable; rc.4 failure evidence identified the exact deterministic recovery assertion, and rc.5 adds allowlisted assertion reporting plus direct exact-container restart; target TrueNAS and HIL remain pending |
...
| 30 | server prerelease workflow plus firmware release packs; stable workflow binds exact marked-unit record to firmware hash/commit and >=72-hour soak | coordinated signed public server/firmware rc.1 and rc.3 assets plus public firmware rc.4 exist; server rc.4 failed before assembly, while coordinated rc.5 and physical certification remain pending their separate evidence |
| 31 | feature branches, logical commit/gate/stable-prohibition policy, and CODEOWNERS | public histories, reviewed audit PR, signed rc.1/rc.3 release tags, and immutable failed server rc.2/rc.4 tags exist; current rc.5 recovery source remains untagged and pending review |
| 32 | final-report fields represented by release manifest/reports/traceability | public rc.1/rc.3 URLs/digests exist; server rc.2 has only a failed-run URL, while rc.4 has a failed-run URL and image evidence but no Release/YAML; rc.5 publication, target deployment, and HIL fields remain unavailable and must not be inferred |
| 33 | definition of done remains open for candidate rc.5 until its coordinated publication, target-TrueNAS recovery testing, and marked-unit certification exist | public rc.3 proves distribution for its immutable seven-service assets only; rc.4 failed closed before assembly, and rc.5's eight-service/no-shell model and stable gates remain fail-closed until their own evidence exists |

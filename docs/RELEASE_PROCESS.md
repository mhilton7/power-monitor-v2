...
firmware rc.24 metadata and artifacts must be published and independently
verified before the server rc.24 tag is created; server
...
Candidate rc.24 keeps the server as durable owner of independently accepted
...
service with exact Decimal tier and fixed-charge semantics. RC24 retains the
RC23 rate-evaluation hardening and also corrects dashboard day totals, the
...
cross-links the compatible firmware release. Coordinated rc.24 publishes under
a new immutable tag without rewriting rc.23 or any earlier release.
...
3. Publish and independently verify the coordinated signed firmware
   `v0.1.0-rc.24` release first. Set the server repository variable
...
4. With the release signing key and local allowed-signers policy configured,
   ...
   git tag -s -m 'PowerMeter V2 0.1.0-rc.24' v0.1.0-rc.24 "$release_commit"
   git verify-tag v0.1.0-rc.24
   test "$(git rev-parse 'v0.1.0-rc.24^{commit}')" = "$release_commit"
   git push origin refs/tags/v0.1.0-rc.24
   ```

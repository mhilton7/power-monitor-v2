...
durable sensor-backlog architecture. Candidate `v0.1.0-rc.25` retains
...
firmware rc.25 metadata and artifacts must be published and independently
verified before the server rc.25 tag is created; server
...
Candidate rc.25 keeps the server as durable owner of independently accepted
...
service with exact Decimal tier and fixed-charge semantics. RC25 retains the
...
`20260821_0019`. The generated rc.25 OpenAPI SHA-256 is
...
cross-links the compatible firmware release. Coordinated rc.25 publishes under
...
3. Publish and independently verify the coordinated signed firmware
   `v0.1.0-rc.25` release first. Set the server repository variable
...
4. With the release signing key and local allowed-signers policy configured,
   ...
   git tag -s -m 'PowerMeter V2 0.1.0-rc.25' v0.1.0-rc.25 "$release_commit"
   git verify-tag v0.1.0-rc.25
   test "$(git rev-parse 'v0.1.0-rc.25^{commit}')" = "$release_commit"
   git push origin refs/tags/v0.1.0-rc.25
   ```

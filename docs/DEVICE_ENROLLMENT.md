...
6. Compare the fingerprint shown by USB and server. Confirm the first signed heartbeat and durable interval.
...
Revoke/unclaim is distinct from clearing readings, formatting microSD, clearing logs, or factory reset. Revocation immediately rejects device authentication but never silently erases history. Re-enrollment does not reset the lifetime sequence of an existing retained identity.

If provisioning fails, keep measurement/storage active where possible and use USB recovery. Temporary server unavailability alone is not a reason to erase or replace device identity.

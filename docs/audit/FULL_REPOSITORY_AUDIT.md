...
| Database | Frozen migrations `0001` through `0007`, additive migrations `20260815_0008` through `20260815_0011`, ORM constraints, transactions, indexes, role initialization, clean/upgrade test definitions, backup, and isolated restore paths |
...
| A-05 | High | Application checks did not create a database-wide mutual-exclusion boundary between immutable raw readings and authenticated permanent-loss ranges; overlapping loss ranges, mutation/deletion of accepted loss evidence, and impossible sample completeness could enter through another writer. | `backend/app/schemas/device.py`, `services/ingestion.py`, `models.py`, migrations `20260815_0008` and `20260815_0010`, ingestion tests | Repaired locally. Migration 0008 locks and preflights the evidence tables, tightens completeness, and adds per-device overlap guards. Migration 0010 rejects UPDATE/DELETE of permanent-loss evidence while preserving INSERT and the frozen raw-reading immutability prerequisite. A real PostgreSQL 17 chain through head 0011, downgrade/re-upgrade, and the direct evidence guards passed; portable migration/guard coverage also passed with PostgreSQL-only cases explicitly skipped outside PostgreSQL. | Existing conflicts intentionally stop migration without rewriting evidence. The tagged public-rc.3 forward-upgrade gate and target deployment remain pending. |
| A-06 | Medium | Official-source validators could advance before durable parsing, and rate-source candidates lacked one exact-home operational workflow with manual fallback, separated review/publish/activate/reject transitions, overlap control, and honest per-home status. | `backend/app/services/rate_sync.py`, `rate_workflow.py`, rate routes/models/schemas, migrations `20260815_0009` and `20260815_0011`, worker jobs, backend/frontend tests | Repaired locally. Validators advance only after durable usable evidence; Linux fsyncs the parent directory; a stranded 304 gets one bounded unconditional recovery fetch. Migration 0009 adds workflow state. Migration 0011 enforces database-backed exact-home manual idempotency, immutable candidate provenance, legal `reviewed -> published -> activated` or `reviewed -> rejected` transitions, unique natural rate plans with serialized version allocation shared by bill/SCE publishing, and deterministic non-overlapping assignments with equal-start rejection. PostgreSQL write locks span its preflight and guard installation. Exact-home status, retries, alerts, and LKG remain fail-closed. | The isolated PostgreSQL 17 clean upgrade and 0011 downgrade/re-upgrade passed; all 20 workflow/concurrency/direct-SQL tests passed. The live SCE page still lacks complete holiday/effective-date evidence and correctly cannot yield a publishable candidate alone. |
...
production build, all 24/24 Vitest tests, and all 36/36 Chromium

...
export function HistoryPage() {
  ...
  const scopeValue = selectedScopeAvailable ? selectedScope : (devices.data?.devices[0] ? `device:${devices.data.devices[0].id}` : circuits.data?.circuits.find((circuit) => circuit.aggregate_mode === 'verified_sum') ? `circuit:${circuits.data.circuits.find((circuit) => circuit.aggregate_mode === 'verified_sum')!.id}` : '');

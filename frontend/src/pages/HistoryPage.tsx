...
export function HistoryPage() {
  ...
  const designatedBranch = verifiedCircuits.find((circuit) => circuit.is_home_total || circuit.purpose === 'whole_home_total');
  ...
  const selectedCircuit = verifiedCircuits.find((circuit) => circuit.id === circuitId);
  const scopedDevices = circuitId
    ? devices.data?.devices.filter((device) => selectedCircuit?.device_ids.includes(device.id)) ?? []
  ...
  return <div className="page history-page">
    ...
    <Card className="history-controls">
      ...
      <div className="filter-row">
        <div className="field"><label htmlFor="history-device">Service branch or sensor</label><select id="history-device" value={scopeValue} onChange={(event) => chooseScope(event.target.value)}>{verifiedCircuits.map((circuit) => <option key={circuit.id} value={`circuit:${circuit.id}`}>{circuit.name}{circuit.is_home_total ? ' · home total' : ''}</option>)}{devices.data?.devices.map((device) => <option key={device.id} value={`device:${device.id}`}>{device.friendly_name}</option>)}</select></div>
    ...
    {history.data && <>
      <div className="history-summary-grid"><Card eyebrow="Selected range" title="Energy"><strong className="stat-value">{numeric(history.data.energy_kwh === null ? null : Number(history.data.energy_kwh), 'kWh')}</strong><small>{dateTime(displayRange.from.toISOString(), timezone)} – {dateTime(displayRange.to.toISOString(), timezone)}</small></Card><Card eyebrow="Selected range" title="Estimated cost"><strong className="stat-value">{money(history.data.cost)}</strong><small>Uses the current rate plan when one is available</small></Card><Card eyebrow="Reading coverage" title="Saved readings"><strong className="stat-value">{percent(history.data.completeness === null ? null : Number(history.data.completeness))}</strong><small>{history.data.missing_ranges.length > 0 ? 'Some readings are missing' : 'No known gaps in this range'}</small></Card></div>
      <Notice>Showing saved readings for {deviceId ? devices.data?.devices.find((device) => device.id === deviceId)?.friendly_name ?? 'the selected sensor' : circuits.data?.circuits.find((circuit) => circuit.id === circuitId)?.name ?? 'the selected service branch'}. Sensors that measure the same electricity must not be added together, because that would count the same usage twice.</Notice>
      {pendingBacklog > 0 && <Notice><strong>Readings are syncing.</strong> {pendingBacklog.toLocaleString()} reading{pendingBacklog === 1 ? ' is' : 's are'} waiting to sync. Live readings can appear before saved History because stored readings must be accepted by the server first.</Notice>}
      <Card title={`${metricDefinition.label} over time`} eyebrow={`${history.data.resolution_seconds}s aggregation · display ${timezone}`} action={<span className="chart-instruction"><ZoomIn aria-hidden="true" /> Drag the handles to zoom</span>} className="history-chart-card">
        ...
        <div className="chart-legend"><span><i className="legend-line" />Saved value</span><span><i className="legend-gap" />Missing reading</span><span><Info aria-hidden="true" /> A measured zero renders at zero; missing values form a gap.</span></div>
      </Card>
      {history.data.missing_ranges.length > 0 && <Card title="Some readings are missing"><div className="gap-list">{history.data.missing_ranges.map((gap) => <div key={`${gap.start}-${gap.end}`}><strong>{dateTime(gap.start, timezone)} – {dateTime(gap.end, timezone)}</strong><span>No saved reading is available for this interval</span></div>)}</div></Card>}
    </>}
  </div>;
}

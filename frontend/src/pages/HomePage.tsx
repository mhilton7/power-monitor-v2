...
export function HomePage() {
  ...
  const devices = useQuery({ queryKey: ['devices', selectedHomeId], queryFn: () => api.devices(selectedHomeId), enabled: Boolean(selectedHomeId), refetchInterval: 30_000 });
  ...
  const history24 = useQuery({
    queryKey: ['history', selectedHomeId, 'home-24h', deviceId],
    queryFn: () => api.history(historyParams(selectedHomeId, deviceId, new Date(now.getTime() - 24 * 60 * 60 * 1000), now, 'power', 300)),
    enabled: Boolean(selectedHomeId && deviceId),
  });
  const daily = useQuery({
    queryKey: ['history', selectedHomeId, 'home-7d', deviceId],
    queryFn: () => api.history(historyParams(selectedHomeId, deviceId, new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000), now, 'energy', 86400)),
    enabled: Boolean(selectedHomeId && deviceId),
  });
  ...
  if (!selectedHomeId) return <div className="page"><h1 className="sr-only">Home</h1><EmptyState title={homeScope.homeScopes.length === 0 ? 'No authorized home' : 'Choose an active home'} detail={homeScope.homeScopes.length === 0 ? 'Your account has no authorized home scope. Home-specific data remains unavailable.' : 'Select a UUID-disambiguated home from the Active home control before loading measurements.'} /></div>;
  ...
  return <div className="home-dashboard">
    ...
    <section className="dashboard-middle" aria-label="Committed usage charts and rate">
      <Card title="Live Usage – Last 24 Hours" eyebrow="Committed sensor intervals" action={<span className="select-chip">kW</span>} className="usage-chart-card">
        {history24.isLoading ? <Loading label="Loading committed intervals" /> : history24.isError ? <ErrorState error={history24.error} /> : history24.data ? <div className="chart-wrap" data-testid="usage-chart"><ResponsiveContainer width="100%" height="100%"><AreaChart title="Committed power over the last 24 hours" desc="Authenticated sensor power intervals. Missing readings render as gaps." data={chartData} margin={{ top: 16, right: 12, bottom: 8, left: -12 }}><defs><linearGradient id="powerFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#65e692" stopOpacity={0.55} /><stop offset="100%" stopColor="#65e692" stopOpacity={0.02} /></linearGradient></defs><CartesianGrid stroke="#33413c" strokeDasharray="3 4" vertical={false} /><XAxis dataKey="epoch" type="number" domain={['dataMin', 'dataMax']} scale="time" minTickGap={70} interval="preserveStartEnd" tickFormatter={(value: number) => chartTick(value, 24, history24.data?.timezone ?? 'America/Los_Angeles')} tick={{ fill: '#9ca9a4', fontSize: 11 }} axisLine={false} tickLine={false} /><YAxis tick={{ fill: '#9ca9a4', fontSize: 11 }} axisLine={false} tickLine={false} width={42} unit=" kW" /><Tooltip content={<UsageTooltip timezone={history24.data.timezone} />} /><Area type="monotone" dataKey="valueKw" stroke="#65e692" strokeWidth={2} fill="url(#powerFill)" connectNulls={false} isAnimationActive={false} /><Brush ariaLabel="Zoom committed power History" dataKey="epoch" height={28} travellerWidth={24} stroke="#65e692" fill="#151d1a" tickFormatter={() => ''} /></AreaChart></ResponsiveContainer></div> : null}
      ...
      <div className="right-stack">
        ...
        <Card title="Alerts & Notifications · all authorized homes" className="compact-alerts">

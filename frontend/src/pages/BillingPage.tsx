...
function tierLabel(value: 'tier_1' | 'tier_2' | 'not_confirmed' | undefined, coverage: string | number | null | undefined): string {
  if (value === 'not_confirmed' || coverage === null || coverage === undefined || Number(coverage) < 1) return 'Tier not confirmed';
...
export function BillingPage() {
  ...
  return <div className="page billing-page">
    <header className="page-heading"><div><p className="eyebrow">Rates and saved home usage</p><h1>Billing</h1><p>See the rate in use, billing-cycle progress, and estimates based on saved sensor readings.</p></div><PermissionGate permission="rates.bill_import"><button type="button" className="button button-primary" onClick={() => setImportOpen(true)}><Upload aria-hidden="true" /> Import rates from SCE bill PDF</button></PermissionGate></header>
    ...
    <Card title="Rate currently used" eyebrow="Current rate plan" className="current-plan-card">
    ...
    <Card title="Current billing cycle" eyebrow="Whole-home bill estimate">
      {account?.current_billing_cycle ? <><div className="plan-title"><div><strong>{dateTime(account.current_billing_cycle.start_utc)} – {dateTime(account.current_billing_cycle.end_utc)}</strong><span>Whole-home branch used: {account.current_billing_cycle.service_branch_name ?? account.home_total_branch?.name ?? 'Main service'}</span></div><StatusPill state={tierLabel(account.current_billing_cycle.tier_state, account.current_billing_cycle.reading_coverage) === 'Tier not confirmed' ? 'waiting' : 'current'} label={tierLabel(account.current_billing_cycle.tier_state, account.current_billing_cycle.reading_coverage)} /></div><dl><div><dt>Saved usage</dt><dd>{pendingNumeric(account.current_billing_cycle.saved_usage_kwh, 'kWh')}</dd></div><div><dt>Reading coverage</dt><dd>{percent(account.current_billing_cycle.reading_coverage === null ? null : Number(account.current_billing_cycle.reading_coverage))}</dd></div><div><dt>Readings waiting to sync</dt><dd>{account.current_billing_cycle.readings_waiting_to_sync?.toLocaleString() ?? 'Count not reported'}</dd></div><div><dt>Tier 1 allowance</dt><dd>{pendingNumeric(account.current_billing_cycle.tier_1_allowance_kwh, 'kWh')}</dd></div><div><dt>Tier 1 remaining</dt><dd>{pendingNumeric(account.current_billing_cycle.tier_1_remaining_kwh, 'kWh')}</dd></div><div><dt>Above Tier 1</dt><dd>{pendingNumeric(account.current_billing_cycle.amount_above_tier_1_kwh, 'kWh')}</dd></div><div><dt>Estimated energy charges</dt><dd>{pendingMoney(account.current_billing_cycle.estimated_energy_charges)}</dd></div><div><dt>Estimated fixed charges</dt><dd>{pendingMoney(account.current_billing_cycle.estimated_fixed_charges)}</dd></div><div><dt>Estimated total</dt><dd>{pendingMoney(account.current_billing_cycle.estimated_total)}</dd></div></dl>{tierLabel(account.current_billing_cycle.tier_state, account.current_billing_cycle.reading_coverage) === 'Tier not confirmed' && <Notice kind="warning">Readings are still syncing, so the current billing tier cannot be confirmed yet. Tier confirmation requires 100% reading coverage.</Notice>}{(account.current_billing_cycle.readings_waiting_to_sync ?? 0) > 0 && <p>Cost estimates will appear after saved sensor readings finish syncing.</p>}</> : <><p>Detailed cycle progress is not reported by this server version.</p><dl><div><dt>Saved usage</dt><dd>{numeric(summary?.billing_cycle.energy_kwh === null || summary?.billing_cycle.energy_kwh === undefined ? null : Number(summary.billing_cycle.energy_kwh), 'kWh')}</dd></div><div><dt>Reading coverage</dt><dd>{percent(summary?.billing_cycle.completeness === null || summary?.billing_cycle.completeness === undefined ? null : Number(summary.billing_cycle.completeness))}</dd></div></dl><Notice kind="warning">Tier not confirmed. Update the server to calculate tier progress from the full Main service billing cycle.</Notice></>}
      {selectedEstimate && <details className="technical-details"><summary>Technical details</summary><dl><div><dt>Energy cost for this branch</dt><dd>{money(selectedEstimate.energy_cost)}</dd></div><div><dt>Rate version</dt><dd>{selectedEstimate.rate_plan_version_id}</dd></div><div><dt>Missing readings</dt><dd>{selectedEstimate.missing_intervals}</dd></div></dl></details>}
    </Card>

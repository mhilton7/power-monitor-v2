...
export function BillingPage() {
  ...
  const tierProgress = cycle && tierBoundary !== null && cycle.saved_usage_kwh !== null
    ? (() => { const usage = Number(cycle.saved_usage_kwh); const boundary = Number(tierBoundary); const scale = Math.max(boundary * 1.25, usage * 1.1, 1); return { usage, scale, usagePercent: Math.min(100, usage / scale * 100), boundaryPercent: Math.min(100, boundary / scale * 100) }; })()
    : null;
  ...
  return <div className="page billing-page">
    ...
    <Card title="Current Billing Cycle" eyebrow="Main service usage">
      {cycle ? <><div className="plan-title"><div><strong>{dateTime(cycle.start_utc)} – {dateTime(cycle.end_utc)}</strong><span>Billing source: {cycle.service_branch_name ?? account?.home_total_branch?.name ?? 'Main service'}</span></div><StatusPill state={tierLabel(cycle.tier_state, cycle.reading_coverage) === 'Tier not confirmed' ? 'waiting' : 'current'} label={tierLabel(cycle.tier_state, cycle.reading_coverage)} /></div><dl><div><dt>Usage to date</dt><dd>{pendingNumeric(cycle.saved_usage_kwh, 'kWh')}</dd></div><div><dt>Reading coverage</dt><dd>{percent(cycle.reading_coverage === null ? null : Number(cycle.reading_coverage))}</dd></div><div><dt>Recovered connection-gap energy</dt><dd>{pendingNumeric(cycle.recovered_gap_energy_kwh ?? cycle.connection_gap_energy_kwh ?? null, 'kWh')}</dd></div><div><dt>Billing adjustment</dt><dd>{pendingNumeric(cycle.billing_adjustment_kwh ?? null, 'kWh')}</dd></div><div><dt>Unresolved connection gaps</dt><dd>{cycle.unresolved_connection_gap_count ?? '—'}</dd></div><div><dt>Unresolved energy</dt><dd>{pendingNumeric(cycle.unresolved_energy_kwh ?? null, 'kWh')}</dd></div></dl>{tierLabel(cycle.tier_state, cycle.reading_coverage) === 'Tier not confirmed' && <Notice kind="warning">Tier not confirmed. Complete reading coverage is required before assigning all usage to a tier.</Notice>}</> : <><p>Billing-cycle progress is not available from this server.</p><dl><div><dt>Usage to date</dt><dd>{numeric(summary?.billing_cycle.energy_kwh === null || summary?.billing_cycle.energy_kwh === undefined ? null : Number(summary.billing_cycle.energy_kwh), 'kWh')}</dd></div><div><dt>Reading coverage</dt><dd>{percent(summary?.billing_cycle.completeness === null || summary?.billing_cycle.completeness === undefined ? null : Number(summary.billing_cycle.completeness))}</dd></div></dl><Notice kind="warning">Tier not confirmed.</Notice></>}
    </Card>
    <Card title="Tier Breakdown" eyebrow="Exact usage and charges">

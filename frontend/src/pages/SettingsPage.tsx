...
export function SettingsPage() {
  ...
  return <div className="page settings-page">
    ...
    <div className="settings-layout"><nav className="settings-nav" aria-label="Settings sections">{visible.map((section) => <button key={section.id} type="button" className={selected === section.id ? 'active' : ''} aria-label={section.label} aria-current={selected === section.id ? 'page' : undefined} onClick={() => setActive(section.id)}>{section.icon}<span>{section.label}</span><ChevronRight aria-hidden="true" /></button>)}</nav><div className="settings-content">
      ...
      {selected === 'health' && <HealthSettings homeId={selectedHomeId} health={health.data} loading={health.isLoading} error={health.error} />}
...
function HomeSettings() {
  ...
  return <Card title="Home & utility" eyebrow="Authoritative schedule, account rules and monitored scope"><form key={query.data.home.id} className="settings-form" onSubmit={submit}>
...
function SensorSettings({ homeScopes, devices, loading, error }: { homeScopes: Array<{ id: string; name: string }>; devices: DeviceDetail[]; loading: boolean; error: unknown }) {
  ...
  const [aggregateHomeId, setAggregateHomeId] = useState('');
  ...
  const scopedAggregateHomeId = onlyHomeId ?? (homeScopes.some((home) => home.id === aggregateHomeId) ? aggregateHomeId : undefined);
  const scopedDevices = scopedAggregateHomeId ? devices.filter((device) => device.home_id === scopedAggregateHomeId) : [];
  const canCreateAggregate = homeScopes.some((home) => devices.filter((device) => device.home_id === home.id).length >= 2);
  ...
  const aggregate = useMutation({
    mutationFn: (payload: { name: string; deviceIds: string[]; homeId: string }) => api.createVerifiedAggregate({ home_id: payload.homeId, name: payload.name, device_ids: payload.deviceIds, confirmation: 'I VERIFIED THESE NON-OVERLAPPING METERS' }),
  ...
  function submitAggregate(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!scopedAggregateHomeId || aggregateDevices.length < 2) return; const form = new FormData(event.currentTarget); aggregate.mutate({ name: formString(form, 'aggregateName'), deviceIds: aggregateDevices, homeId: scopedAggregateHomeId }); }
...
function DataPrivacySettings() {
  return <><Card title="Data retention & sources" eyebrow="Explicit privacy boundaries"><dl><div><dt>Electrical History</dt><dd>Authenticated PZEM readings and immutable loss evidence only; a bill never supplies History.</dd></div><div><dt>Bill uploads</dt><dd>Processed in bounded temporary memory for allowlisted rate facts. Original PDF bytes are never retained, encrypted or otherwise; extracted working drafts can be deleted.</dd></div><div><dt>Official SCE evidence</dt><dd>Disposable unpublished or rejected candidates can be deleted. Published rate versions and their source hashes remain immutable provenance.</dd></div><div><dt>Firmware artifacts</dt><dd>Binary bytes can be removed after all intended sensor deployments finish. Version, digest, deployment and audit metadata remain.</dd></div><div><dt>Account security</dt><dd>Passwords are one-way hashed. Password changes, resets, disablement and deletion revoke active sessions.</dd></div></dl><Notice>No database or sensor reset is available here. Sensor-history formatting requires a separate prepare/commit confirmation and preserves enrollment, credentials and sequence evidence.</Notice></Card><Card title="Exports & audit evidence"><div className="dialog-actions"><PermissionGate permission="history.export"><a className="button button-secondary" href="/history">Open History export</a></PermissionGate><PermissionGate permission="logs.view"><a className="button button-secondary" href="/settings?section=logs">Open audit diagnostics</a></PermissionGate><PermissionGate permission="rates.bill_import"><a className="button button-secondary" href="/billing">Open rate-only bill import</a></PermissionGate></div></Card></>;
}

function HealthSettings({ homeId, health, loading, error }: { homeId: string; health: Awaited<ReturnType<typeof api.health>> | undefined; loading: boolean; error: unknown }) {
  ...
  return <Card title="Advanced system health" eyebrow="Exact evidence from central services"><div className="health-heading"><Activity aria-hidden="true" /><div><strong>Overall system</strong><span>Checked {dateTime(health.generated_at)}</span></div><StatusPill state={overall} /></div><dl><div><dt>Application version</dt><dd>{health.version}</dd></div><div><dt>Internal home ID</dt><dd><code>{homeId || 'No active home'}</code> <button type="button" className="text-button" onClick={() => void copyHomeId()} disabled={!homeId}>Copy</button>{copyState === 'copied' && <span role="status"> Copied</span>}{copyState === 'failed' && <span role="alert"> Copy unavailable</span>}</dd></div></dl><div className="service-grid"><article><div><strong>Database</strong><StatusPill state={health.database === 'reachable' ? 'healthy' : 'unhealthy'} /></div><p>{health.database}</p></article><article><div><strong>Protocol</strong><StatusPill state="healthy" /></div><p>{health.protocol}</p></article>{health.sensors.map((sensor) => <article key={sensor.device_id}><div><strong>Sensor {sensor.device_id.slice(0, 8)}</strong><StatusPill state={sensor.state} /></div><p>PZEM {sensor.pzem_status}; storage {sensor.storage_status}; backlog {sensor.backlog ?? 'unavailable'}.</p></article>)}<article><div><strong>Hardware certification</strong><StatusPill state="warning" /></div><p>{health.physical_hardware_certification}</p></article></div></Card>;
}

...
function SensorSettings({ homeScopes, devices, loading, error }: { homeScopes: Array<{ id: string; name: string }>; devices: DeviceDetail[]; loading: boolean; error: unknown }) {
  ...
  const saveBranch = useMutation({
    mutationFn: (payload: { name: string; description: string | null; purpose: 'electrical_section' | 'whole_home_total'; deviceIds: string[] }) => editingBranch
      ? api.updateCircuit(editingBranch.id, { name: payload.name, description: payload.description, purpose: payload.purpose, is_home_total: payload.purpose === 'whole_home_total', device_ids: payload.deviceIds, confirmation: 'I VERIFIED THESE NON-OVERLAPPING METERS' })
      : api.createCircuit({ home_id: activeHomeId, name: payload.name, description: payload.description, purpose: payload.purpose, is_home_total: payload.purpose === 'whole_home_total', device_ids: payload.deviceIds, confirmation: 'I VERIFIED THESE NON-OVERLAPPING METERS' }),
  ...
  function submitBranch(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!activeHomeId || branchDevices.length === 0) return; const form = new FormData(event.currentTarget); saveBranch.mutate({ name: formString(form, 'branchName'), description: formString(form, 'branchDescription') || null, purpose: branchPurpose, deviceIds: branchDevices }); }
  function closeEnrollment() { enrollment.reset(); setEnrollmentHomeId(''); setEnrollOpen(false); }
  function openNewBranch() { setEditingBranch(undefined); setBranchDevices([]); setBranchPurpose('electrical_section'); setBranchOpen(true); }
  function openBranch(branch: Circuit) { setEditingBranch(branch); setBranchDevices(branch.device_ids); setBranchPurpose(branch.is_home_total || branch.purpose === 'whole_home_total' ? 'whole_home_total' : 'electrical_section'); setBranchOpen(true); }
...
function HealthSettings({ homeId, health, devices, loading, error }: { homeId: string; health: Awaited<ReturnType<typeof api.health>> | undefined; devices: DeviceDetail[]; loading: boolean; error: unknown }) {
  ...
  return <>
    ...
    <Card title="Sensor synchronization" eyebrow="Saved-reading progress">
      <div className="service-grid">{health.sensors.map((sensor) => {
        const detail = devices.find((device) => device.id === sensor.device_id);
        const sync = sensor.synchronization ?? detail?.synchronization;
        return <article key={sensor.device_id}><div><strong>{sensor.device_name ?? detail?.friendly_name ?? `Sensor ${sensor.device_id.slice(0, 8)}`}</strong><StatusPill state={sensor.state} /></div><p>{sensor.backlog === null ? 'Waiting count not reported' : sensor.backlog === 0 ? 'All saved readings are synchronized' : `${sensor.backlog.toLocaleString()} readings waiting to sync`}</p><dl><div><dt>Firmware</dt><dd>{sensor.firmware_version ?? detail?.firmware_version ?? 'Not reported'} · build {sensor.firmware_build_id ?? detail?.firmware_build_id ?? 'not reported'}</dd></div><div><dt>Server acknowledged</dt><dd>{sensor.acknowledgement ?? detail?.acknowledgement ?? 'Not reported'}</dd></div></dl><details className="technical-details"><summary>Technical details</summary><dl><div><dt>Sensor ID</dt><dd>{sensor.device_id}</dd></div><div><dt>Firmware digest</dt><dd>{abbreviated(sensor.firmware_digest ?? detail?.firmware_digest)}</dd></div><div><dt>Protocol</dt><dd>{sensor.protocol ?? detail?.protocol ?? 'Not reported'}</dd></div><div><dt>Boot partition</dt><dd>{sensor.boot_partition ?? detail?.boot_partition ?? 'Not reported'}</dd></div><div><dt>Last successful OTA</dt><dd>{sensor.last_successful_ota ?? detail?.last_successful_ota ?? 'Not reported'}</dd></div><div><dt>Stored sequence range</dt><dd>{sensor.oldest_sequence ?? detail?.oldest_sequence ?? '—'}–{sensor.newest_sequence ?? detail?.newest_sequence ?? '—'}</dd></div><div><dt>Last attempted batch</dt><dd>{sync?.last_batch_start ?? '—'}–{sync?.last_batch_end ?? '—'}</dd></div><div><dt>Selected records / bytes</dt><dd>{sync?.selected_record_count ?? 'Not reported'} / {sync?.serialized_bytes === null || sync?.serialized_bytes === undefined ? 'Not reported' : bytes(sync.serialized_bytes)}</dd></div><div><dt>HTTP result</dt><dd>{sync?.http_result ?? 'Not reported'}</dd></div><div><dt>Last accepted sequence</dt><dd>{sync?.last_accepted_sequence ?? 'Not reported'}</dd></div><div><dt>Missing prefix</dt><dd>{sync?.missing_prefix_status ?? 'Not reported'}</dd></div><div><dt>Last error</dt><dd>{sync?.last_error ?? 'None reported'}</dd></div><div><dt>Queue drain rate</dt><dd>{sync?.queue_drain_rate === null || sync?.queue_drain_rate === undefined ? 'Not reported' : `${sync.queue_drain_rate} readings/min`}</dd></div></dl></details></article>;
      })}</div>
    </Card>
  </>;
}

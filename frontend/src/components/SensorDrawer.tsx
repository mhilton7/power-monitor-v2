  ...
  return <>
    <Dialog open={open} title={device.friendly_name} description={`Device ${device.device_fingerprint}`} onClose={onClose} wide>
      ...
      <div className="detail-grid">
        <section><h3><ServerCog aria-hidden="true" /> Device</h3><dl><div><dt>Firmware</dt><dd>{device.firmware_version ?? 'Not available'}</dd></div><div><dt>Protocol</dt><dd>{device.protocol}</dd></div><div><dt>Meter variant</dt><dd>{device.pzem_variant}</dd></div><div><dt>Credential</dt><dd>{device.credential_fingerprint ? `${device.credential_fingerprint.slice(0, 16)}… · key ${device.credential_key_version ?? 'unknown'}` : 'Not available'}</dd></div><div><dt>Last reboot</dt><dd>{device.last_reboot_reason ?? 'Not available'}</dd></div></dl></section>
    ...
    <Dialog open={configureOpen} title={`Configure ${device.friendly_name}`} description="Friendly name and measurement scope are stored centrally and applied through authenticated configuration." onClose={() => setConfigureOpen(false)}><form className="settings-form" onSubmit={submitConfiguration}><div className="field"><label htmlFor="sensor-friendly-name">Friendly name</label><input id="sensor-friendly-name" name="friendlyName" defaultValue={device.friendly_name} required maxLength={120} /></div><div className="field"><label htmlFor="sensor-measurement-scope">Measurement scope</label><select id="sensor-measurement-scope" name="measurementScope" value={scopeDraft} onChange={(event) => setScopeDraft(event.target.value)}><option value="">Leave current server value unchanged</option><option value="energy_only">Energy only</option><option value="allocated_account">Allocated account</option><option value="full_account">Full account</option></select><small>The device list does not expose the current scope, so this UI never guesses it.</small></div>{scopeDraft === 'full_account' && <div className="field"><label htmlFor="sensor-full-scope-confirmation">Type I VERIFIED THIS METER COVERS THE FULL ACCOUNT</label><input id="sensor-full-scope-confirmation" name="measurementScopeConfirmation" required pattern="I VERIFIED THIS METER COVERS THE FULL ACCOUNT" autoComplete="off" /></div>}{scopeDraft === 'allocated_account' && <div className="field"><label htmlFor="sensor-allocated-scope-confirmation">Type I VERIFIED THIS ALLOCATION SCOPE</label><input id="sensor-allocated-scope-confirmation" name="measurementScopeConfirmation" required pattern="I VERIFIED THIS ALLOCATION SCOPE" autoComplete="off" /></div>}{configure.isError && <Notice kind="warning">{configure.error instanceof Error ? configure.error.message : 'Sensor configuration failed.'}</Notice>}<div className="dialog-actions"><button type="button" className="button button-secondary" onClick={() => setConfigureOpen(false)}>Cancel</button><button type="submit" className="button button-primary" disabled={configure.isPending}>{configure.isPending ? 'Saving…' : 'Save sensor'}</button></div></form></Dialog>

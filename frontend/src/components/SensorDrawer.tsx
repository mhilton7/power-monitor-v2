  ...
  return <>
    <Dialog open={open} title={device.friendly_name} description={`Device ${device.device_fingerprint}`} onClose={onClose} wide>
      ...
      <div className="detail-grid">
        ...
        <section><h3><HardDrive aria-hidden="true" /> microSD</h3><dl><div><dt>State</dt><dd>{device.storage_status}</dd></div><div><dt>Capacity / free</dt><dd>Not reported</dd></div><div><dt>Stored sequences</dt><dd>{device.oldest_sequence ?? '—'}–{device.newest_sequence ?? '—'}</dd></div><div><dt>Server acknowledged</dt><dd>{device.acknowledgement}</dd></div></dl></section>
        ...
        <section><h3><RefreshCw aria-hidden="true" /> Command & OTA</h3><dl><div><dt>Last command</dt><dd>{device.last_command ? `${device.last_command.type} · ${device.last_command.state}` : 'None'}</dd></div><div><dt>Progress</dt><dd>{device.last_command ? `${device.last_command.progress_percent}%` : 'Not available'}</dd></div><div><dt>OTA release</dt><dd>Not reported</dd></div></dl></section>
      </div>
      <h3 className="section-label">Sensor controls</h3>
      <div className="control-grid">
        <PermissionGate permission="sensors.configure"><button type="button" className="control-button" onClick={() => command.mutate({ type: 'diagnostics_snapshot' })}><Activity aria-hidden="true" /><span>Run diagnostics<small>Collect redacted evidence</small></span></button><button type="button" className="control-button" onClick={() => command.mutate({ type: 'sync_now' })}><RefreshCw aria-hidden="true" /><span>Sync now<small>Prioritize committed backlog</small></span></button><button type="button" className="control-button" onClick={() => command.mutate({ type: 'network_self_test' })}><Wifi aria-hidden="true" /><span>Network self-test<small>DNS, TLS and server checks</small></span></button></PermissionGate>
    ...
    <Dialog open={configureOpen} title={`Configure ${device.friendly_name}`} description="Identity, presentation, monitoring, and measurement-scope settings are stored centrally and audited." onClose={() => setConfigureOpen(false)}><form className="settings-form" onSubmit={submitConfiguration}><div className="field"><label htmlFor="sensor-friendly-name">Friendly name</label><input id="sensor-friendly-name" name="friendlyName" defaultValue={device.friendly_name} required maxLength={120} /></div><div className="field"><label htmlFor="sensor-location">Location</label><input id="sensor-location" name="location" defaultValue={device.location ?? ''} maxLength={120} placeholder="Main electrical panel" /></div><div className="field"><label htmlFor="sensor-notes">Notes</label><textarea id="sensor-notes" name="notes" defaultValue={device.notes ?? ''} maxLength={500} rows={3} /></div><div className="field"><label htmlFor="sensor-display-order">Display order</label><input id="sensor-display-order" name="displayOrder" type="number" min="0" max="10000" step="1" defaultValue={device.display_order} required /></div><div className="appearance-options"><label><input type="checkbox" name="showOnDashboard" defaultChecked={device.show_on_dashboard} /><span><strong>Show on dashboard</strong><small>Hide this sensor card without deleting evidence.</small></span></label><label><input type="checkbox" name="includeInAggregate" defaultChecked={device.include_in_aggregate} /><span><strong>Eligible for aggregates</strong><small>Verified aggregate rules still apply.</small></span></label><label><input type="checkbox" name="monitoringEnabled" defaultChecked={device.monitoring_enabled} /><span><strong>Operational monitoring</strong><small>Disabling alerts and availability status never stops authenticated ingestion.</small></span></label></div><div className="field"><label htmlFor="sensor-measurement-scope">Measurement scope</label><select id="sensor-measurement-scope" name="measurementScope" value={scopeDraft} onChange={(event) => setScopeDraft(event.target.value)}><option value="">Leave current server value unchanged</option><option value="energy_only">Energy only</option><option value="allocated_account">Allocated account</option><option value="full_account">Full account</option></select><small>Scope changes require explicit verification; the current scope is {device.measurement_scope.replaceAll('_', ' ')}.</small></div>{scopeDraft === 'full_account' && <div className="field"><label htmlFor="sensor-full-scope-confirmation">Type I VERIFIED THIS METER COVERS THE FULL ACCOUNT</label><input id="sensor-full-scope-confirmation" name="measurementScopeConfirmation" required pattern="I VERIFIED THIS METER COVERS THE FULL ACCOUNT" autoComplete="off" /></div>}{scopeDraft === 'allocated_account' && <div className="field"><label htmlFor="sensor-allocated-scope-confirmation">Type I VERIFIED THIS ALLOCATION SCOPE</label><input id="sensor-allocated-scope-confirmation" name="measurementScopeConfirmation" required pattern="I VERIFIED THIS ALLOCATION SCOPE" autoComplete="off" /></div>}{configure.isError && <Notice kind="warning">{configure.error instanceof Error ? configure.error.message : 'Sensor configuration failed.'}</Notice>}<div className="dialog-actions"><button type="button" className="button button-secondary" onClick={() => setConfigureOpen(false)}>Cancel</button><button type="submit" className="button button-primary" disabled={configure.isPending}>{configure.isPending ? 'Saving…' : 'Save sensor'}</button></div></form></Dialog>

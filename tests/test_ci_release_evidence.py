...
def test_public_release_selector_executes_against_paginated_fail_closed_fixtures() -> None:
    ...
    assert selected.stdout.strip() == "v0.1.0-rc.10"
...
def test_release_predecessor_version_check_rejects_equal_or_newer_versions() -> None:
    ...
    assert validate("v0.1.0-rc.10", "v0.1.0-rc.11").returncode == 0
    assert validate("v0.1.0-rc.11", "v0.1.0-rc.11").returncode != 0
    assert validate("v0.1.0-rc.12", "v0.1.0-rc.11").returncode != 0
    assert validate("v0.1.0", "v0.1.0-rc.11").returncode != 0
    assert validate("not-a-version", "v0.1.0-rc.11").returncode != 0
...
def test_candidate_notes_describe_workflow_output_without_claiming_source_publication() -> None:
    ...
    assert "power-monitor-v2-v0.1.0-rc.11.yaml" in normalized
    ...
    assert "firmware `v0.1.0-rc.11`" in normalized
    assert "9bfae34ae18fac2f9c2e6937f20c4c65c56a721c44d92ab10c079aaf2dd3a620" in normalized
    ...
    assert "Public server rc.10 remains installable" in normalized
    assert "Firmware rc.11 must be published and verified" in normalized
...
def test_rc3_recovery_docs_separate_failed_rc2_forward_upgrade_and_publication() -> None:
    ...
    assert "therefore selects public rc.10" in release_process
    ...
    assert "Candidate firmware rc.11" in firmware

...
def test_candidate_notes_describe_workflow_output_without_claiming_source_publication() -> None:
    ...
    assert "power-monitor-v2-v0.1.0-rc.25.yaml" in normalized
    assert "Alembic head: `20260821_0019`" in normalized
    assert "firmware tag: `v0.1.0-rc.25`, build number `28`" in normalized
    assert "pm-telemetry/2.0.0" in normalized
    assert "f40aed47eb572db1d328e3130fd0a86e6a8c9c123ba244d4cb90db3a4dd039bb" in normalized
...
def test_release_process_requires_merge_then_verified_signed_annotated_tag() -> None:
    ...
    create = "git tag -s -m 'PowerMeter V2 0.1.0-rc.25'"
    verify = "git verify-tag v0.1.0-rc.25"
    push = "git push origin refs/tags/v0.1.0-rc.25"

...
def test_public_release_selector_executes_against_paginated_fail_closed_fixtures() -> None:
    ...
    pages = [
        [
            {
                "draft": False,
                "published_at": "2026-08-17T09:10:00Z",
                "prerelease": True,
                "tag_name": "v0.1.0-rc.13",
            },
            {
                "draft": False,
                "published_at": "2026-08-17T06:12:35Z",
                "prerelease": True,
                "tag_name": "v0.1.0-rc.12",
            },
            {
                "draft": False,
                "published_at": "2026-08-17T04:08:04Z",
                "prerelease": True,
                "tag_name": "v0.1.0-rc.11",
            },
            {
                "draft": False,
                "published_at": "2026-08-17T00:19:05Z",
                "prerelease": True,
                "tag_name": "v0.1.0-rc.10",
            },
            {
                "draft": False,
                "published_at": "2026-08-16T19:51:08Z",
                "prerelease": True,
    ...
    assert selected.stdout.strip() == "v0.1.0-rc.13"
...
def test_release_predecessor_version_check_rejects_equal_or_newer_versions() -> None:
    ...
    assert validate("v0.1.0-rc.13", "v0.1.0-rc.14").returncode == 0
    assert validate("v0.1.0-rc.14", "v0.1.0-rc.14").returncode != 0
    assert validate("v0.1.0-rc.15", "v0.1.0-rc.14").returncode != 0
    assert validate("v0.1.0", "v0.1.0-rc.14").returncode != 0
    assert validate("not-a-version", "v0.1.0-rc.14").returncode != 0
...
def test_candidate_notes_describe_workflow_output_without_claiming_source_publication() -> None:
    ...
    assert "power-monitor-v2-v0.1.0-rc.26.yaml" in normalized
    assert "Alembic head: `20260821_0019`" in normalized
    assert "firmware tag: `v0.1.0-rc.26`, build number `29`" in normalized
    assert "pm-telemetry/2.0.0" in normalized
    assert "22d5025b80dbee17ccd714beef4649c87e7169364ec8d3ea652fa9be0c74490d" in normalized
...
def test_release_process_requires_merge_then_verified_signed_annotated_tag() -> None:
    ...
    create = "git tag -s -m 'PowerMeter V2 0.1.0-rc.26'"
    verify = "git verify-tag v0.1.0-rc.26"
    push = "git push origin refs/tags/v0.1.0-rc.26"
...
def test_rc3_recovery_docs_separate_failed_rc2_forward_upgrade_and_publication() -> None:
    ...
    assert "therefore selects public rc.13" in release_process
